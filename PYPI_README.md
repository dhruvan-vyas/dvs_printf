...
`dvs_printf` features an interactive Command Line Interface (CLI) built into the package (`__main__.py`). You can execute it directly from your terminal using `python -m dvs_printf` to test animation styles, view color tables, launch task loaders, or open the interactive help guide without writing code.

### Command Syntax

```bash
# Run full interactive feature showcase & demo
python -m dvs_printf

# Display CLI help menu & available options
python -m dvs_printf --help

# Test a specific animation style (e.g. glitch, matrix, wave, typing)
python -m dvs_printf --style glitch
python -m dvs_printf -s matrix

# Run targeted component demos (loader, spinner, colors, help)
python -m dvs_printf --demo loader
python -m dvs_printf -d spinner
python -m dvs_printf -d colors
python -m dvs_printf -d help
```
...
| `-s`, `--style` | `STYLE_NAME` | Runs an animation demo for the specified style key (`glitch`, `matrix`, `typing`, `wave`, etc.). |
| `-d`, `--demo` | `DEMO_NAME` | Executes a targeted component demo: `loader`, `spinner`, `colors`, `help`, or `all`. |
