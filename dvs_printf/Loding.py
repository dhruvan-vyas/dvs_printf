...
def loding(t1, Text, lodingchar):
    ...
    for i in range(len(x) - 20):
        print(
            short
            + " " * (len(x) - i - 1)
            + f"][ {' ' if len(str(int(i*3.33)))==1 else ''}{int(i*3.33)}%]",
            end="\r",
            flush=True,
        )
        sleep(speed)
    ...
    for i in range(len(x) - 20, len(x) - 5):
        print(
            short + " " * (len(x) - i - 1) + f"][ {int(i*3.33)}%]", end="\r", flush=True
        )
        sleep(speed)
        short = short + x[i]
        if t1.is_alive():
...
#### lodingChar
    ...
    try:
        ...
        lodingChar = (
            str(lodingChar)[0]
            if (len(str(lodingChar)) != 0)
            and (lodingChar != " ")
            and (lodingChar[0] not in ["", "\n", "\r", "\b", "\t", "\a", "\f", "\v"])
            else "-"
        )
...
print("skfjh;ashg;afgjhj;ahfgh;adfhghadfhghjk")
