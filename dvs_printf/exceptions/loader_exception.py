...
# class LoaderValueError(dvs_BaseException, ValueError):
...
#         return (
#                 f"{PURPLE}LoaderValueError{RESET}: The speed value must be of type {GRAY}int{RESET} or {GRAY}float{RESET}, "
...
encountered within the LoadingBar module.
"""
...
class LoaderValueError(dvs_BaseException, ValueError):
    ...
    def __init__(self, error_value: object, message: Optional[str] = None, keyWord: str = "config"):
        """
        Initializes the LoaderValueError.
    ...
    def __generate_message__(self) -> str:
        ...
        if self._message:
            return f"{PURPLE}LoaderValueError{RESET}: {self._message}"
        else:
            return (
                f"{PURPLE}LoaderValueError{RESET}: Invalid value encountered during LoadingBar configuration or runtime. \n"
                f"{PURPLE}Received{RESET}: {GRAY}{self.value!r}{RESET} (type: {GRAY}{self._error_value_type.__name__}{RESET})."
            )



class LoaderAttributeError(dvs_BaseException, AttributeError):
    """
    ...
    def __init__(self, error_value: object, expected_type: str, keyWord: str):
        """
        Initializes the LoaderAttributeError.
    ...
    def __generate_message__(self) -> str:
        ...
        return (
            f"{PURPLE}LoaderAttributeError{RESET}: Invalid type for configuration keyword: {PEACH}{self.keyWord}{RESET}.\n"
            f"{PURPLE}Received{RESET} (type {received_type_name}): {GRAY}{self.value!r}{RESET}, {PURPLE}Expected Type{RESET}: {GRAY}{self.expected_type}{RESET}\n"
        )
