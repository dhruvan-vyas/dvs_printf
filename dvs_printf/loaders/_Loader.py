...
# from .loader_frems     import get_bar_style
...
#         # bar_gradient_style: Optional[str] = None,
...
#     return Loder.run(target, *args, **kwargs, timeout = timeout)


# class LoadingBar(LoadingBarConfig):
#     """
#     A customizable and reusable loading screen that works as a context manager and a function.
#     """
...
#         self.task_completed: Event = Event()
...
#         """Pre-calculates all colors, gradients, and bar styles."""
...
#         # print(len(self.full_block), self.full_block)
...
#         borders = self.bar_borders or self.default_borders
...
#             # if isinstance(self.bar_gradient_style, (list, tuple)):
#                 # self.gradient_colors = Colors(self.bar_gradient_style)._get_ansi_color(get_list=True)[0]
#             # elif isinstance(self.bar_gradient_style, str):
#                 # self.gradient_colors = get_gredinat_style(self.bar_gradient_style)

#             if self.is_gradient:
#                 self.gradient_colors = Colors((self.bar_color + self.bar_color[::-1]))
#             else:
#                 _write(f':{[self.bar_color]}:')
#                 _flush()
#                 self.gradient_colors = Colors(self.bar_color)
...
#         # Initial bar length (will be updated if full_screen_mode is enabled)
#         self.bar_length = 40
#         self.use_bar_len = self.bar_length - self.partial_char_len

#     def _get_refresh_rate(self) -> float:
#         """Determines the refresh rate based on the timeout duration."""
...
#         """
#         Compile a dynamic loader function for performance. \\
#         Returns the `_run_animation_loop` function ready to call. 

#         def _run_animation_loop(self): 
#             Unified loader logic for both gradient and non-gradient modes, adapted from user code.
#         """
...
#         elif self.task_thread.is_alive(): # {"and progress < 100" if not self.inject_progress else ""}:
#             e = "Timeout ERROR"
...
#         _flush()
#     '''
#         # * Map class attributes to local variables 
#         # for clarity and to match the original function's scope 
#         exec_globals = {
#             'self'             : self,
#             'time'             : time,
#             'sleep'            : sleep,
...
#         # Local scope where compiled code runs
#         local_scope = {}
...
#     def run(self, target: Callable, *args, timeout:int=None, **kwargs, ):
#         """
#         Runs the given function in a separate thread while the animation displays.
#         This is a blocking call that returns the target function's result.


#         Runs the target function with all console output suppressed, while
#         providing dedicated write and flush functions to bypass the suppression.
        
#         This function aggressively redirects both Python-level streams and
#         lower-level file descriptors to a dummy stream, ensuring that no
#         output can escape to the console while the animation is running.
        
#         The provided _write and _flush functions are the *`only`* way for the
#         target function to print to the console.
#         """
#         self.result = None
#         self.exception = None
...
#         try:
#             # # Step 1: Redirect the low-level file descriptors (fd 1 for stdout, fd 2 for stderr).
#             # # This catches all output from C libraries and other low-level functions.
#             os.dup2(null_fd, sys.stdout.fileno())
#             os.dup2(null_fd, sys.stderr.fileno())

#             # # Step 2: Redirect the high-level Python streams for good measure.
#             # # This prevents the Python interpreter from getting confused.
...
#             # Step 3: Run the user's target function.
#             # We pass our dedicated _write and _flush functions as kwargs so the
#             # target function can use them to bypass the suppression.
...
#             self.exception = e
#         finally:
#             # # Step 4: Restore all original streams and file descriptors to their
#             # # original state, regardless of whether the task succeeded or failed.
...
#         """
#         Makes the instance callable, providing a clean, single-line API.
#         Example: result = loading_bar_instance(my_task, arg1)
#         """
...
def _run_animation_loop():
    ...
    def run(self, target: Callable, *args, timeout:int=None, **kwargs, ) -> Any:
        ...
        if self.exception:
            ...
            print([self.title_str, self.title_text, self.exception])
